...
function DatabasePage() {
  ...
  if (status === "loading") {
    return (
      <div className="text-sm text-muted-foreground">Starting database…</div>
    );
  }
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-foreground/70">
            PGlite runs Postgres in WebAssembly, right in your browser. Query
            tables, inspect schemas, and experiment safely—no server required.
            Try commands like{" "}
            <code className="rounded bg-muted px-1 py-0.5 text-xs">\dt</code> to
            list tables.
          </p>
        </CardHeader>
